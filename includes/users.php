...
// the only place that knows they live in this table. users.php (admin-only
// CRUD screen) calls these instead of querying directly. Session/capability
// logic (currentUser(), userCan(), login/logout) lives in includes/auth.php
...
const USER_PROFILE_COLUMNS = 'id, email, username, first_name, last_name, phone, line_id, avatar_path, role, created_at';
...
// partials/header.php) needs them and only currentUser() is guaranteed
// loaded that early, not this file's DB-backed CRUD functions below.

function getAllUsers(): array
{
    return db()->query('SELECT ' . USER_PROFILE_COLUMNS . ' FROM mblog_staff ORDER BY created_at')->fetchAll();
}

// setup.php's whole safety gate rests on this being 0 — a lightweight
// COUNT(*) instead of count(getAllUsers()) so that check doesn't need to
...
{
    $stmt = db()->prepare('SELECT ' . USER_PROFILE_COLUMNS . ' FROM mblog_staff WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch();
...
// collects email/password (users.php's "add new user", setup.php) so
...
// profile.php.
function generateUsernameFromEmail(string $email): string
{
    ...
    $username = $base;
    $suffix = 2;
    while (userUsernameExists($username)) {
...
function createUser(string $email, string $username, string $password, string $role = 'author', array $profile = []): int
{
...
function updateUser(int $id, string $email, string $username, string $role, array $profile = []): void
{
...
// Separate from updateUser() — the edit form's password field is optional
...
// previous file for this user regardless of its old extension — same
// single-current-file-per-slot pattern as settings.php's saveSiteAsset()
// for the site logo/favicon, just keyed by user id instead of a fixed slot
// name. Returns the new relative path, or null if this submit didn't
// include a file.
