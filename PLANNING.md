...
- **เหลือรอ**: URL สวย (`/article/slug` แทน `article.php?slug=`) — Google ให้น้ำหนัก URL อ่านง่ายกว่าเล็กน้อยด้วย **แนวทาง routing ตัดสินใจแล้ว** (คุยกัน 2026-07-27) เป็น "`.htaccess` rewrite เขียนเอง" (ไม่ใช่ router กลางไฟล์เดียวแบบ WordPress) เพราะ mBlog รู้จำนวน/ชนิดหน้าคงที่อยู่แล้ว (article/category/tag/page) ไม่ต้องมี engine generate rule อัตโนมัติแบบ WP ที่ต้องรองรับ post type ไม่จำกัดจากปลั๊กอิน — เขียน rewrite rule มือ 4-5 บรรทัดพอ ไฟล์ PHP เดิมแทบไม่ต้องแตะ logic เลย (ยังอ่าน `$_GET['slug']`/`$_GET['page']` เหมือนเดิม)
  ...
  - **พักไว้ก่อน ยังไม่ทำ**: เดิมกังวลว่าโฮสจริงตอน deploy จะเปิด `AllowOverride`/`mod_rewrite` ให้ได้หรือเปล่า — **หมดข้อกังวลนี้แล้ว** ตอนนี้ตัดสินใจโฮสจริงเป็น AWS EC2 แล้ว (คุมเซิร์ฟเวอร์เอง เปิดได้แน่นอน — ดู Phase 10) แต่ยังไม่เร่งด่วนพอจะเริ่มทำตอนนี้ รอคิวถัดจากงาน production readiness อื่นก่อน
...
- **หมายเหตุค้าง**: ยังไม่ได้ผูก Apache `ErrorDocument` กับ 404/500 กลาง เพราะ path ขึ้นกับ document root จริงตอน deploy — ตอนนี้รู้แล้วว่าจะ deploy จริงบน AWS EC2 (ดู Phase 10) ทำได้ตอนตั้งเครื่องจริงเลย ไม่ต้องรอข้อมูลอะไรเพิ่มอีก
...
- [x] **เลือกโฮสจริงแล้ว: AWS EC2** (คุยกัน 2026-08-08, ยังไม่ได้ลงมือจริง) — instance เดียวรัน Apache+PHP+MySQL ในตัว (ไม่ใช้ RDS/Docker/Lambda) เพราะเข้ากับสถาปัตยกรรมปัจจุบันตรงที่สุด **โค้ด PHP ไม่ต้องแก้เลยสักบรรทัด** (มีแค่งานตั้งค่าเซิร์ฟเวอร์ครั้งแรก — ดู checklist ด้านล่าง) — **เหตุผลตัด Lambda ออก**: ต้องรื้อระบบไฟล์อัปโหลด/session/`.htaccess` ทั้งหมด (Lambda ไม่มี disk ถาวร) และถ้าจะคง MySQL (ไม่ใช่ DynamoDB) ต้องมี VPC+NAT Gateway (~$32/เดือน) แพงกว่า EC2 เฉยๆ เสียอีก — จุดประสงค์ "ฟรีตลอดไป (AWS Always Free)" เลยหมดความหมาย (EC2/RDS ไม่ได้อยู่ใน Always Free อยู่แล้ว มีแค่ 12 เดือนแรกของบัญชีใหม่ที่ฟรี) — **เหตุผลตัด Docker ออก (ตอนนี้)**: เพิ่มความซับซ้อนที่ยังไม่จำเป็นสำหรับสเกลนี้ (ต้องคิดเรื่อง volume persistence ของ `uploads/`/`backups/` เพิ่ม) ถ้าอยากได้ environment local/prod ตรงกันเป๊ะค่อยพิจารณาทีหลังได้ ไม่ใช่ของค้าง
  - **งานตั้งเครื่องครั้งแรก (checklist ตอนลงมือจริง)**: ลง PHP extension เดิม (`pdo_mysql`/`curl`/`zip`/`gd`/`mbstring`) → คัดลอก `config.php` ปรับ DB credentials ของเครื่องนั้น (`config.example.php` ออกแบบมาสำหรับเคสนี้อยู่แล้ว ไม่ใช่เรื่องใหม่) → ตั้ง `APP_ENV='production'` → เปิด `AllowOverride All`+`mod_rewrite` ใน Apache (จำเป็นเพราะมี `.htaccess` deny-all ที่ `backups/`/`uploads/`) → ตั้งสิทธิ์เขียนไฟล์ให้ user เว็บเซิร์ฟเวอร์เข้าถึง `uploads/`/`backups/`/`logs/` ได้ → รัน SQL migration ใน `database/` เหมือนตอนตั้ง local
  - **ค่าใช้จ่ายคร่าวๆ** (ราคา on-demand region US, เช็ค ณ 2026-08) — 12 เดือนแรกของบัญชีใหม่เกือบ $0 ด้วย Free Tier (EC2 t2/t3.micro 750 ชม./เดือน + public IPv4 750 ชม./เดือน + EBS ฟรีถึง 30GB) — หลังจากนั้นประมาณ **$13-14/เดือน** (t3.micro, 1GB RAM) หรือ **~$21/เดือน** (t3.small, 2GB RAM — พอมีระบบหุ้นเพิ่มเข้ามาอาจต้องใช้ระดับนี้) รวม EC2+EBS 20-30GB+public IPv4 (ค่า public IPv4 ~$3.65/เดือน บังคับมีตั้งแต่ปี 2024 แม้ใช้งานปกติ) — data transfer out ฟรี 100GB/เดือนตลอดไป (ไม่ใช่แค่ปีแรก) ปกติเพียงพอสำหรับบล็อกขนาดนี้
  - **โดเมน** — ซื้อที่ไหนก็ได้ (Route 53 หรือ registrar อื่น) ~$12-14/ปี ไม่จำเป็นต้องใช้ Route 53 hosted zone เสมอไป (ประหยัด $0.50/เดือน ถ้าใช้ DNS ของ registrar เดิมจัดการ record เอง) — ต้องมี **Elastic IP** ผูกกับ instance ก่อนถึงจะชี้โดเมนได้เสถียร (public IP ปกติเปลี่ยนทุกครั้งที่ instance stop/start)
  - **ทดลองก่อนมีโดเมน** — ไม่ต้องซื้อโดเมนเลยก็ทดสอบได้ทันที AWS ให้ public IP + public DNS name อัตโนมัติ (เช่น `ec2-x-x-x-x.compute-1.amazonaws.com`) ใช้เข้าเว็บได้เลย แค่เปิดพอร์ต 80 ใน Security Group ก่อน (จุดที่คนพลาดบ่อยสุดตอนเทสครั้งแรก)
- [ ] HTTPS (Let's Encrypt) บนโฮสจริง — ทำได้ตอนตั้ง EC2 จริง ติดตั้ง Certbot บนเครื่องเองตรงๆ ไม่ต้องพึ่งบริการ SSL อื่นของ AWS
...
- **Phase 11 — ฟีเจอร์บทความเพิ่มเติม** ✅ เสร็จ 5/6 (SEO override, Sticky, Scheduled publishing, Post expiration; Private post เพิ่มทีหลัง 2026-08-07) เหลือ Hierarchical pages (พักไว้รอ pretty URL) — ดูหัวข้อ 13 สำหรับรายละเอียดที่เบี่ยงจากแผนเดิม (โดยเฉพาะ sticky ที่ดีไซน์เปลี่ยนไปมากหลังคุยกัน 3 รอบ) และบั๊ก 2 ตัวที่เจอ+แก้ระหว่างทำ (settings cache ไม่ sync, PHP/MySQL timezone ไม่ตรงกัน)
...
- Phase 7 (คอมเมนต์), บทความล็อกด้วยรหัสผ่าน (หัวข้อ 14) — auth (Phase 3) พร้อมใช้แล้วทั้งคู่ เหลือแค่สร้างตัวฟีเจอร์เอง
